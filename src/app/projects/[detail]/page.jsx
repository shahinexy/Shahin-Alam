...
const ProjectDetail = ({params}) => {
  ...
  <ul>
    ...
    <li className="text-secondary">{project.project_name}</li>
            ...
            <div>
            <Image src={project?.image} alt="Logo"  width={1000} height={800}></Image>
            </div>
            <div className="mt-5">
                ...
                <div className="mt-4">
                    ...
                    <ul className="list-disc ml-8 space-y-3">{
                        project.features.map((item, idx) => <li key={item.idx}>{item}</li>)
                        }</ul>
                </div>
                ...
                {
                  project?.tools.map((tool, idx) => <p key={idx} className="shadow-lg shadow-black/50 px-3"> {tool} </p>)
                }
                </div>

                <div className="grid sm:grid-cols-3 grid-cols-1 gap-3 mt-12">
                    <Link href={`${project.live_link}`} target="_blank" className="w-full">
                    ...
                    <Link href={`${project.client_repo_link}`} target="_blank" className="w-full">
                    <button className="w-full flex items-center gap-2 justify-center border py-2 text-xl font-medium hover:scale-95 duration-500">
                    ...
                    {
                        project?.server_repo_link && <Link href={`${project.server_repo_link}`} target="_blank" className="w-full">
                        <button className="w-full flex items-center gap-2 justify-center border py-2 text-xl font-medium hover:scale-95 duration-500">
